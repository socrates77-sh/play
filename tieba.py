...
# 2020/07/11  v2.7  iteration change
...
stars_obs = ['孙允珠', '周慧敏', '王祖贤', '张雪迎', '杨超越',  '黑木明纱', '李倩蓉', '徐冬冬', '朝比奈彩',
             '坎迪斯', '安布罗休', 'evanrachelwood', 'lilyjames']
...
stars = [
    ...
    '何穗', '奚梦瑶', '辛芷蕾', '张蓝心', '钟楚曦', '宋祖儿', '张芷溪',
    '孙怡', '户田惠梨香', '白百合', '文咏珊', '桥本环奈', '林珍娜', '蛯原友里',
    'amberheard', 'mackenziefoy', 'emiliaclarke', 'sashaluss',
    '盖尔加朵', '吴谨言', '西野七濑', '山本舞香', '佐久间由衣', '松本若菜', '土屋太凤', '广濑铃',
    '万茜', '松井爱莉', 'blackpink', 'lisa', 'jennie', '吉高由里子', '长泽雅美', 
    '迪丽热巴', ]
...
class TiebaAll():
    ...
    def page_urls(self):
        ...
        # print(urls)
        if len(urls) <= 1:
            print('cannot access %s' % first_page_url)
            sys.exit(1)
        return urls
...
def download_all_stars(stars, last_id, test, start_idx):
    '''
    下载一个指定贴吧，在某一日期前所有的图片
    :param stars: 贴吧名表
    :param last_id: 最后已下载的id
    ...
    '''
    count = 0
    for i in range(start_idx, len(stars)):
        l = stars[i]
        count += download(l, last_id, test)
        print('[%d/%d]' % (i+1, len(stars)))
    print('----------------------------------------')
...
def main():
    ...
    download_all_stars(stars, last_id, False, start_idx)
    wait_any_key()
