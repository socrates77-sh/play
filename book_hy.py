...
def save_file(title, txt):
    out_file = os.path.join(ROOT_DIR+r'\..', title + '.txt')
    with open(out_file, 'w+') as f:
        f.write(txt)
    print('(F) %s' % title)


def do_single_file(file_name, title):
    ...
    # save_file(title, txt)
    return clean_title(title) + 3*'\n' + txt


def get_files(index_file):
    full_name_index = index_file
    with open(full_name_index, 'r', errors='ignore') as f:
        index_text = f.read()

    # p = re.compile(r'<A HREF="([0-9].+?.htm).*?"\s*?>.+?</A>.*?<', re.M)
    # rt = re.findall(p, index_text)
    # if rt:
    #     return rt

    p = re.compile(
        r'<a href="([ax0-9\-]+?.htm|[a-z0-9]+?-[0-9]+?.html).*?"\s*?>(.+?)</a>.*?<', re.M | re.I)
    rt = re.findall(p, index_text)
    if rt:
        return rt


def do_multi_file(index_file, title):
    full_path = os.path.join(ROOT_DIR, index_file.replace('/', '\\'))
